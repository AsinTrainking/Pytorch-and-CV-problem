...
datasets_sizes = {
    x: len(image_datesets[x])

    for x in ['train','val']

}

class_names = image_datesets['train'].classes

print("Class name", class_names)

print("Dataset sizes:",datasets_sizes)
    ...
    with torch.no_grad():
        for i,(inputs, labels) in enumerate(dataloaders['val']):
            ...
            for j in range(inputs.size()[0]):
                ...
                ax.set_title(f'predict: {class_names[preds[j]]}')
...
print('Model weight save to resnet18_fixed_feature_extractor.pth')
